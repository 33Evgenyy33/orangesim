...
!function($){
	...
	$usof.field['font'] = {
		...
		init: function(options){
			...
			} else if (this.curFont != 'none' && this.curFont.indexOf(',') != -1) {
				this.$preview.css('font-family', this.curFont + '');
			}
			this.$select.select2();
	...
	$usof.Group.prototype = {
		...
		init: function(elm, options){
			...
			this.$btnDelGroup = this.$field.find('.usof-form-group-delete');
			this.groupName = this.$field.data('name');
			this.groupIndex = this.$field.data('index');
			this.isHB = (this.$field.data('hb') != undefined) ? this.$field.data('hb') : 0;
			...
			if (this.isAccordion) {
				this.paramsTitle = (this.$field.data('params_title') != undefined) ? decodeURIComponent(this.$field.data('params_title')) : '';
				if (this.paramsTitle != undefined && this.paramsTitle != '') {
					$.each(this.$field.find('.usof-form-wrapper'), function(index, elm){
						var $groupFields = $(elm),
							$groupFieldsTitle = $groupFields.find('.usof-form-wrapper-title'),
							groupFieldsIndex = $groupFields.data('index');
						$.each($groupFields.find('.usof-form-row'), function(index, elm){
							var $groupField = $(elm),
								name = $groupField.data('name');

							$usof.instance.fields[name].on('change', function(){
								var paramsTitleResult = this.paramsTitle;
								$.each($groupFields.find('.usof-form-row'), function(index, elm){
									var $groupField = $(elm),
										name = $groupField.data('name'),
										nameInGroup = name.replace(this.groupName+'_'+groupFieldsIndex+'_', '');
									if (paramsTitleResult.indexOf('{{'+nameInGroup+'}}') !== -1){
										paramsTitleResult = paramsTitleResult.replace('{{'+nameInGroup+'}}', $usof.instance.getValue(name));
									}

								}.bind(this));
								$groupFieldsTitle.html(paramsTitleResult);
							}.bind(this));
				...
				this.$sections = this.$field.find('.usof-form-wrapper');
				this.$sectionTitles = this.$field.find('.usof-form-wrapper-title');
				this.$sectionTitles.live('click', function(event){
					this.$sections = this.$field.find('.usof-form-wrapper');
					var $parentSection = $(event.target).parent();
					if ($parentSection.hasClass('active')) {
						$parentSection.removeClass('active').children('.usof-form-wrapper-content').slideUp();
					} else {
						var $previousActive = this.$sections.filter('.active');
						$previousActive.removeClass('active').children('.usof-form-wrapper-content').slideUp();
						$parentSection.addClass('active').children('.usof-form-wrapper-content').slideDown();
					}
			...
			if (this.isSortable) {
				...
				this.$dragControls = this.$field.find('.usof-form-group-drag');
		...
		groupAdd: function(){
			...
			this.ajaxData.index = this.groupIndex;
			$.ajax({
				type: 'POST',
				url: $usof.ajaxUrl,
				dataType: 'json',
				data: this.ajaxData,
				success: function(result){
					this.$btnAddGroup.before(result.data.paramsHtml);
					$usof.instance.initFields(this.$field);
					$usof.instance.fireFieldEvent(this.$field, 'beforeShow');
					$usof.instance.fireFieldEvent(this.$field, 'afterShow');
					// TODO: rewrite code so this code is reused START
					for (var fieldName in $usof.instance.showIfDeps) {
						if (!$usof.instance.showIfDeps.hasOwnProperty(fieldName) || $usof.instance.fields[fieldName] === undefined) continue;
						$usof.instance.fields[fieldName].on('change', function(field){
							$usof.instance.updateVisibility(field.name);
						});
						$usof.instance.updateVisibility($usof.instance.fields[fieldName].name);
					}
					if ( ! this.isHB){
						for (var fieldId in $usof.instance.fields) {
							if (!$usof.instance.fields.hasOwnProperty(fieldId)) continue;
							$usof.instance.fields[fieldId].off('change').on('change', function(field, value){
								...
								$usof.instance.valuesChanged[field.name] = value;
							});
						}

						// TODO: rewrite code so this code is reused END
						var $addedGroup = this.$field.find('.usof-form-wrapper').last();
						$.each($addedGroup.find('.usof-form-row, .usof-form-wrapper, .usof-form-group'), function(index, elm){
							var $field = $(elm),
								name = $field.data('name');
							$usof.instance.valuesChanged[name] = $usof.instance.fields[name].getValue();
						});
		...
		groupDel: function($group){
			...
			if ( ! this.isHB) {
				...
				$.each($group.find('.usof-form-row, .usof-form-wrapper, .usof-form-group'), function(index, elm){
					var $field = $(elm),
						name = $field.data('name');
					$usof.instance.valuesChanged[name] = null;
				});
			}
		...
		_dragStart: function(event){
			event.stopPropagation();
			//this.$sections = this.$field.find('.usof-form-wrapper');
			//this.$sections.removeClass('active').children('.usof-form-wrapper-content').slideUp();
			this.$draggedElm = $(event.target).closest('.usof-form-wrapper');
		...
		_isSortable: function(elm){
			return this._hasClass(elm, 'usof-form-wrapper');
		}
	};
		...
		initFields: function($container){
			...
			$.each($container.find('.usof-form-row, .usof-form-wrapper, .usof-form-group'), function(index, elm){
				var $field = $(elm),
					...
					$showIf = $field.find(isRow ? '> .usof-form-row-showif' : '> .usof-form-wrapper-content > .usof-form-wrapper-showif');
			...
			$.each(groupElms, function(index, elm){
				var $field = $(elm);
				$field.usofGroup(elm);
			});
