...
function us_breadcrumbs() {
	...
	$text['category'] = us_translate( 'Category: %s' ); // text for a category page
	$text['search'] = us_translate( 'Search results for &#8220;%s&#8221;' ); // text for a search results page
	$text['tag'] = us_translate( 'Tag: %s' ); // text for a tag page
	$text['author'] = us_translate( 'Author is %s' ); // text for an author page
	...
	if ( is_home() || is_front_page() ) {
		...
		if ( is_category() ) {
			...
			if ( isset( $post_type->labels->name ) ) {
				...
				if ( function_exists( 'bbp_is_single_user' ) and bbp_is_single_user() ) {
					echo us_translate( 'Profile', 'bbpress' );
				} else {
		...
		if ( get_query_var( 'paged' ) AND ! ( get_post_type() == 'topic' OR get_post_type() == 'forum' ) ) {
			...
			echo us_translate( 'Page' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo ')';
			}
		}
