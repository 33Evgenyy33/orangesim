		...
		if ( preg_match( '~\[us_gallery.+?\]|\[us_image_slider.+?\]|\[gallery.+?\]~', $the_content, $matches ) ) {
			...
			if ( ! empty( $us_post_slider_size ) ) {
				if ( preg_match( '~layout=\"[a-z]+\"~', $gallery ) ) {
					$gallery = preg_replace( '~img_size=\"[a-z]+\"~', 'img_size="' . $us_post_slider_size . '"', $gallery );
				} else {
...
function us_display_maintenance_page() {
	...
	if ( class_exists( 'SitePress' ) ) {
		$is_translated = apply_filters( 'wpml_element_has_translations', NULL, $maintenance_page->ID, 'page' );

		// Switch to default language if Maintenace page isn't translated yet
		if ( ! $is_translated ) {
			global $sitepress;
			$lang = $sitepress->get_default_language();
			$sitepress->switch_lang( $lang );
		}
		$maintenance_page = get_post( icl_object_id( $maintenance_page->ID, 'page', TRUE ) );
	}
 ...
 * @return array
 */
function us_get_posts_titles_for( $post_type, $force_no_cache = TRUE ) {
	// Caching results
	static $result = array();
