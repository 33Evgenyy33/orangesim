...
function us_enqueue_fonts() {
	...
	foreach ( $prefixes as $prefix ) {
		$font_options[] = explode( '|', us_get_option( $prefix . '_font_family', 'none' ), 2 );
	}
...
function us_styles() {
	...
	if ( defined( 'US_DEV' ) AND US_DEV ) {
		...
		$css_file_dir = $wp_upload_dir['basedir'] . '/us-assets';
		$css_file_dir = str_replace( '\\', '/', $css_file_dir );
		$site_url_parts = parse_url( site_url() );
		$css_file_suffix = ( ! empty( $site_url_parts['host'] ) ) ? $site_url_parts['host'] : '';
		$css_file_suffix .= ( ! empty( $site_url_parts['path'] ) ) ? str_replace( '/', '_', $site_url_parts['host'] ) : '';
		$css_file_suffix = ( ! empty( $css_file_suffix ) ) ? $css_file_suffix : '';
		$css_file = $css_file_dir . '/' . $css_file_suffix . '.css';
		...
		if ( file_exists( $css_file ) ) {
			$css_file_uri = $wp_upload_dir['baseurl'] . '/us-assets/' . $css_file_suffix . '.css';
			// Removing protocols for better compatibility with caching plugins and services
			$css_file_uri = str_replace( array( 'http:', 'https:' ), '', $css_file_uri );
			wp_enqueue_style( 'us-theme', $css_file_uri, array(), US_THEMEVERSION, 'all' );
		}
	} else {
...
function us_jscripts() {
	...
	if ( us_get_option( 'gmaps_api_key', '' ) != '' ) {
		wp_register_script( 'us-google-maps', '//maps.googleapis.com/maps/api/js?key=' . us_get_option( 'gmaps_api_key', '' ), array(), '', FALSE );
	} else {
