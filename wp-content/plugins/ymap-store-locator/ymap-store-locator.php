...
//	if ( ( get_post_type() == 'ymapsl_stores' ) || ( isset( $_GET['post_type'] ) && ( $_GET['post_type'] == 'ymapsl_stores' ) ) ) {
    ...
    if ($hook == 'post-new.php' || $hook == 'post.php') {
        if ('ymapsl_stores' === get_post_type()) {
            ...
            wp_register_script('yandex-maps-admin-js', 'https://api-maps.yandex.ru/2.1/?lang=ru_RU', array('jquery'), '2.1.65', true);
                ...
                <table id="ymapsl-store-hours" class="ymapsl-twentyfour-format">
                    ...
                    foreach ($opening_days as $index => $day) {
                        ...
                        <tr>
                            ...
                            <td id="ymapsl-hours-<?php echo esc_attr($index); ?>" class="ymapsl-opening-hours"
                                ...
                                if ($hour_count > 0) {
                                    ...
                                    while ($i < $hour_count) {
                                        ...
                                        if ((count($hours) == 2) && (!empty($hours[0])) && (!empty($hours[1]))) {
                                            ?>
                                            <div class="ymapsl-current-period <?php if ($i > 0) {
                                                echo 'ymapsl-multiple-periods';
                                            } ?>">
                                                <input name="ymapsl[hours][<?= $index ?>_open][]"
                                                       class="ymapsl-open-hour" value="<?= $hours[0] ?>">
                                                <span> - </span>
                                                <input name="ymapsl[hours][<?= $index ?>_close][]"
                                                       class="ymapsl-close-hour" value="<?= $hours[1] ?>">
...
//		if ( array_key_exists( 'ymapsl_' . $field_key, $_POST ) ) {
        ...
        update_post_meta($post_id, '_ymapsl_' . $field_key, trim($_POST['ymapsl_' . $field_key]));
//		}
    }
}
...
//		wp_enqueue_style( 'ymapsl-select2-css', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.0/css/select2.min.css', false, YMAPSL_VERSION_NUM );
        wp_register_style('ymapsl-frontend-css', $style_url . 'frontend-style.css', false, YMAPSL_VERSION_NUM);
        ...
        wp_register_script('ymaps-frontend-js', 'https://api-maps.yandex.ru/2.1/?lang=ru_RU', array('jquery'), '2.1.65', true);
        wp_register_script('ymapsl-select2-js', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.1/js/select2.min.js', array('jquery'), YMAPSL_VERSION_NUM, true);
        wp_register_script('ymapsl-frontend-js', $script_url . 'frontend-script.js', array(
            'jquery',
            'ymaps-frontend-js',
            'ymapsl-select2-js'
        ), YMAPSL_VERSION_NUM, true);
...
function ymapsl_frontend()
{
    ...
    wp_enqueue_style('ymapsl-frontend-css');
    wp_enqueue_script('ymaps-frontend-js');
    wp_enqueue_script('ymapsl-select2-js');
    wp_enqueue_script('ymapsl-frontend-js');
