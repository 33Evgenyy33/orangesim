...
//		add_action( 'woocommerce_init', array( $this, 'handler_wc_init' ), 500 );
		add_action( 'wp_loaded', array( $this, 'handler_wc_init' ), 500 );
	...
	public function handler_ajax_calc_fee_pay_order()
	{
		...
		if ( $order->get_id() == $order_id ) 
		{
			...
			else 
			{
				...
				$status = isset( $status_defined[ $curr_status ] ) ? $status_defined[ $curr_status ] : __( 'unknown status', self::TEXT_DOMAIN );
...
//					$this->tax_classes[ sanitize_title( $class) ] = $class;
	...
	public function calculate_gateway_fees_order( $order_id, WC_Order_Add_Fees &$add_fee_order, $ignore_recalc_option = false )
	{
		...
		{
			$add_fee_order->update_payment_method( $this->payment_gateway_key, $this->gateways[ $this->payment_gateway_key ]->title );
