...
//							'cache_wsdl' => WSDL_CACHE_NONE
				...
				public function calculate_shipping( $package = array() ) {
					if ( ! array_key_exists( 'post_data', $_REQUEST ) ) {
						return;
					}
...
//					$short_fias = $get_array['fias'];
//					$formatted_fias = substr($short_fias,0, 1).' '.chunk_split(substr($short_fias, 1), 3, ' ').'000 000 000 000';
//					print_r($get_array);

//					file_put_contents( $_SERVER['DOCUMENT_ROOT'] . "/logs/calc.txt", print_r( $formatted_fias, true )."\r\n", FILE_APPEND | LOCK_EX );
					...
					if ( $city == 'г Москва' ) {
						...
						$this->add_rate( $rate );
					} else if ( $city == 'г Санкт-Петербург' ) {
						$rate = array(
							'id'    => $this->id,
							'label' => $this->title,
							'cost'  => 500
						...
						if ( ! array_key_exists( 'fias', $get_array ) ) {
							return;
						}
						file_put_contents( $_SERVER['DOCUMENT_ROOT'] . "/logs/calc.txt", print_r( $get_array['fias'], true ) . "\r\n", FILE_APPEND | LOCK_EX );

						$fias_num = $get_array['fias'];
						if ( $fias_num === '' ) {
							return;
						}
...
//						file_put_contents( $_SERVER['DOCUMENT_ROOT'] . "/logs/calc.txt", print_r( $this->getRecipientData()->return->List, true ), FILE_APPEND | LOCK_EX );
						if ( $this->getRecipientData() == null ) {
							return;
						}
