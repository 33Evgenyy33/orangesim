...
class ACA_WC_Filtering_ShopOrder_Product extends ACA_WC_Filtering_ShopOrder {
	...
	public function filter_by_wc_product_title( $where, WP_Query $query ) {
		...
		if ( $query->is_main_query() ) {
			$where .= $wpdb->prepare( "AND om.meta_value = %d AND (om.meta_key = '_product_id' OR om.meta_key = '_variation_id')", $this->get_filter_value() );
		}
		...
		switch ( $this->column->get_product_property() ) {
			case 'sku':
				$values = $wpdb->get_results(
					...
					INNER JOIN {$wpdb->prefix}woocommerce_order_itemmeta om ON ( p.ID = om.meta_value AND  ( om.meta_key = '_variation_id' OR om.meta_key = '_product_id' ) )
