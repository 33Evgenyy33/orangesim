...
function ushb_load_header_settings( $header_settings ) {
	...
	if ( empty( $us_header_id ) ) {
		...
		} elseif ( function_exists( 'is_woocommerce' ) AND is_woocommerce() AND is_singular() AND ( ! isset( $usof_options['header_product_defaults'] ) OR $usof_options['header_product_defaults'] == 0 ) ) {
			$us_header_id = isset( $usof_options['header_product_id'] ) ? $usof_options['header_product_id'] : '';
		...
		} elseif ( ( is_archive() OR is_search() ) AND ( ! isset( $usof_options['header_archive_defaults'] ) OR $usof_options['header_archive_defaults'] == 0 ) ) {
...
function ushb_create_post_types() {
	...
	function ushb_us_header_columns_content( $column_name, $post_ID ) {
		if ( $column_name == 'used_in' ) {
			...
			if ( isset( $usof_options['header_product_id'] ) AND $usof_options['header_product_defaults'] == 0 AND $usof_options['header_product_id'] == $post_ID ) {
			...
			foreach ( $used_in['options'] as $where ) {
				echo '<a href="' . admin_url() . 'admin.php?page=us-theme-options#hb" title="' . __( 'Go to Theme Options', 'us' ) . '">' . __( 'Theme Options', 'us' ) . ' > ' . _x( 'Headers', 'site top area', 'us' ) . '</a> > ' . $where . '<br>';
			}
