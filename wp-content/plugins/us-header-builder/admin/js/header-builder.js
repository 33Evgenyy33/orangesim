		...
		_updateElementPlaceholder: function($elm, id, values){
			...
			if (type == 'text' && (values.text || values.icon)) {
				...
				$.each(values, function(key, value){
					if (key == 'style' || key == 'color' || key == 'hover' || key.substr(0, 7) == 'custom_' || key.substr(0, 4) == 'size'  || key.substr(0, 8) == 'hide_for' || key == 'design_options') return;
					if (value != '') socialsHtml += '<i class="fab fa-' + key + '"></i>';
				});
