			...
			if ( substr( $elm, 1, 7 ) == 'wrapper' ) {
				...
				if ( $type == 'text' AND isset( $values['text'] ) AND ( ! empty( $values['text'] ) OR ! empty( $values['icon'] ) ) ) {
					...
					foreach ( $values as $key => $value ) {
						if ( $key == 'style' OR $key == 'color' OR $key == 'hover' OR substr( $key, 0, 7 ) == 'custom_' OR substr( $key, 0, 4 ) == 'size' OR substr( $key, 0, 8 ) == 'hide_for' OR $key == 'design_options' ) {
							continue;
						}
						if ( ! empty( $value ) ) {
							$socialsOutput .= '<i class="fab fa-' . $key . '"></i>';
						}
					}
