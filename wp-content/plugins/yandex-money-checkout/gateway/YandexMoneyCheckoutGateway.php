	        ...
	        foreach ($items as $item) {
		        ...
		        if ( $sim_card == 1346 ) {
			        ...
			        if ($balance_name == '€5'){
				        ...
				        $builder->addReceiptItem(
					        'Тариф Orange, регистрация в сети Оператором',
					        $amount,
					        $quantity,
					        $this->getYmTaxRate($taxes)
				        );
				        $itemTotal = 400 * $quantity;
				        $amount = $itemTotal / $quantity;
				        $builder->addReceiptItem(
					        'Баланс сим-карты',
					        $amount,
			        ...
			        $amount = $itemTotal / $quantity;
			        $builder->addReceiptItem(
				        'Тариф Orange, регистрация в сети, активация пакета Mundo Оператором',
				        $amount,
				        $quantity,
				        $this->getYmTaxRate($taxes)
			        );
			        ...
			        $amount = $itemTotal / $quantity;
			        $builder->addReceiptItem(
				        'Стоимость пакета Mundo ' . $package_name,
				        $amount,
				        $quantity,
				        $this->getYmTaxRate($taxes)
			        );
