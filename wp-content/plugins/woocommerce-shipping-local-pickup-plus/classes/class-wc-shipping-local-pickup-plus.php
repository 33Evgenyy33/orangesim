...
class WC_Shipping_Local_Pickup_Plus extends WC_Shipping_Method
{
    ...
    public function __construct()
    {
        ...
        if ($this->is_available(array())) {
            ...
            add_action('woocommerce_after_template_part', array(
                $this,
                'review_order_shipping_pickup_location'
            ), 10, 4);
            ...
            if (SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0()) {
                ...
                add_action('woocommerce_add_order_item_meta', array(
                    $this,
                    'associate_order_item_to_cart_item'
                ), 10, 3);
                add_action('woocommerce_add_shipping_order_item', array(
                    $this,
                    'checkout_add_shipping_order_item'
                ), 10, 3);
            }

            // add local pickup location selection row to recurring shipping rates tables in Subscriptions v2.0.18+
            add_action('woocommerce_subscriptions_after_recurring_shipping_rates', array(
                $this,
                'subscription_review_recurrent_shipping_pickup_location'
            ), 10, 4);
            // prevents Subscriptions firing a notice if a discount is applied to a cart containing a subscription
            add_filter('woocommerce_subscriptions_validate_coupon_type', array(
                $this,
                'validate_subscription_pickup_coupon'
            ), 10, 2);
            ...
            add_filter('woocommerce_shipping_packages', array(
                $this,
                'remove_duplicate_cost_for_split_packages_at_same_location'
            ));

            // use the pickup location as the taxable address
            add_filter('woocommerce_customer_taxable_address', array($this, 'taxable_address'));

            add_filter('woocommerce_per_product_shipping_skip_free_method_local_pickup_plus', array(
                $this,
                'per_product_shipping_skip_free_method'
            ));
        }
        ...
        if (is_admin()) {
            ...
            add_action('woocommerce_admin_order_data_after_shipping_address', array(
                $this,
                'admin_order_pickup_location'
            ));

            // save shipping method options
            add_action('woocommerce_update_options_shipping_local_pickup_plus', array(
                $this,
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = array(
            ...
            'checkout_pickup_location_styling' => array(
                ...
                'type' => 'select',
                'options' => array(
                    'select' => __('Dropdown', 'woocommerce-shipping-local-pickup-plus'),
                    'radio' => __('Radio Buttons', 'woocommerce-shipping-local-pickup-plus')
                ),
                'default' => 'select',
    ...
    public function process_admin_options()
    {
        ...
        // reserved fields
        $countries = isset($_POST[$this->id . '_country']) ? array_map(array(
            $this,
            'clean_location_field'
        ), $_POST[$this->id . '_country']) : array();
        $ids = isset($_POST[$this->id . '_id']) ? array_map(array(
            $this,
            'clean_location_field'
        ), $_POST[$this->id . '_id']) : array();
        $costs = isset($_POST[$this->id . '_cost']) ? array_map(array(
            $this,
            'clean_amount_field'
        ...
        foreach (array_keys($pickup_fields) as $field_name) {
            $posted_fields[$field_name] = isset($_POST[$this->id . '_' . $field_name]) ? array_map(array(
                $this,
                'clean_location_field'
            ), $_POST[$this->id . '_' . $field_name]) : array();
        }
...
//	                print_r($location_store);
        ...
        if ($chosen_pickup_location && isset($chosen_pickup_location['taid']) && $chosen_pickup_location['taid']) {
            ...
            echo '<p style="margin-top:10px;font-weight:700;margin-bottom:0;">Убедительная просьба уточнять режим работы по контактным телефонам. Благодарим за понимание!</p>';
            echo '</div>';

        } elseif ($chosen_pickup_location && !$chosen_pickup_location['taid']) {
            echo '<div>';
            echo '<p style="margin-bottom: 0;color: #fff;font-weight: 600;padding-bottom: 9px;font-size: 17px;text-align: center"><span style="color: #f00;">Внимание!</span> По наличию сим-карт обращайтесь по телефону:<br> ' . $chosen_pickup_location['phone'] . '</p>';
            echo '<span><span class="wpsl-location-address-label" >Пункт выдачи: </span>' . $chosen_pickup_location['company'] . '</span><br/>';
            echo '<span><span class="wpsl-location-address-label" >Город: </span>' . $chosen_pickup_location['city'] . '</span><br/>';
            echo '<span><span class="wpsl-location-address-label" >Адрес: </span>' . $chosen_pickup_location['address_1'] . '</span><br/>';
            echo '<span><span class="wpsl-location-address-label" >Телефон: </span>' . $chosen_pickup_location['phone'] . '</span><br/>';
     ...
     */
    public
    function after_checkout_validation(
        $posted
    )
    {
     ...
     */
    public
    function cart_shipping_method_full_label(
        $full_label, $method
    )
    {
     ...
     */
    public
    function associate_order_item_to_cart_item(
        $item_id, $item_or_values, $cart_item_key
    )
    {
     ...
     */
    public
    function checkout_new_shipping_order_item(
        $shipping_item_id, $shipping_item, $order_id
    )
    {
            ...
            if (isset($shipping_methods[$shipping_item_id]['method_id'])
                ...
                foreach ($packages[$package_index]['contents'] as $cart_item_id => $values) {
                    ...
                    if ($order_item_id) {
                        ...
                        wc_update_order_item_meta($order_item_id, __('Пункт выдачи', 'woocommerce-shipping-local-pickup-plus'), $this->get_formatted_address_helper($pickup_location, true));
                    }
                }
            }
        }
     ...
     */
    public
    function checkout_add_shipping_order_item(
        $order_id, $shipping_item_id, $package_index
    )
    {
     ...
     */
    public
    function order_pickup_location(
        $order
    )
    {
        ...
        if (count($pickup_locations) > 0) {
            echo '<div>';
            echo '<header class="title"><h3>' . _n('Пункт выдачи', 'Pickup Locations', count($pickup_locations), 'woocommerce-shipping-local-pickup-plus') . '</h3></header>';
        }
     ...
     */
    public
    function email_pickup_location(
        $order
    )
    {
     ...
     */
    public
    function email_pickup_location_plain(
        $order
    )
    {
     ...
     */
    private
    function get_order_pickup_locations(
        $order
    )
    {
     ...
     */
    public
    function per_product_shipping_skip_free_method(
        $skip
    )
    {
        return false;
     ...
     */
    public
    function admin_order_hide_itemmeta(
        $hidden
    )
    {
     ...
     */
    public
    function admin_order_pickup_location()
    {
                ...
                jQuery(document).ready(function ($) {
                    ...
                    $('.edit_pickup_location select').change(function () {
                        ...
                        $.each(orderLineItems, function (index, lineItemId) {
                            ...
                            var meta_key = $('.item[data-order_item_id="' + lineItemId + '"] input[type="text"][value="<?php _e('Пункт выдачи', 'woocommerce-shipping-local-pickup-plus'); ?>"]');
                            ...
                            if (meta_key.length > 0) {
                                ...
                                $('.item[data-order_item_id="' + lineItemId + '"] .display_meta tr').each(function (index, element) {
                                    if ('<?php _e('Пункт выдачи', 'woocommerce-shipping-local-pickup-plus'); ?>:' == $(element).find('th').text()) {
     ...
     */
    public
    function admin_process_shop_order_meta(
        $post_id, $post
    )
    {
     ...
     */
    public
    static function get_chosen_shipping_methods()
    {
     ...
     */
    public
    function get_checkout_pickup_location_styling()
    {
     ...
     */
    public
    function get_chosen_pickup_location_id(
        $package_index
    )
    {
     ...
     */
    public
    function has_chosen_pickup_location(
        $package_index
    )
    {
        // check for numeric because '0' is a valid location id, whereas null/false/'' are not
     ...
     */
    private
    function get_pickup_location_by_id(
        $id
    )
    {
     ...
     */
    private
    function get_pickup_address_fields(
        $country
    )
    {
     ...
     */
    private
    function get_formatted_address_helper(
        $address, $one_line = false, $show_cost = false
    )
    {
     ...
     */
    private
    function get_products_by_allowed_category(
        $contents
    )
    {
     ...
     */
    private
    function get_cost_for_location(
        $address
    )
    {
     ...
     */
    private
    function get_location_cost_range()
    {
     ...
     */
    private
    function get_percentage_amount(
        $value
    )
    {
