...
//		print_r( $insert_vals );
		// insert
		$i_count = 0;
		foreach ( $insert_vals[ $this->primary_key ] as $key => $value ) {
			$sql    = $this->db->prepare( "SELECT `$this->primary_key` FROM `$this->table_name` WHERE `$this->primary_key` = '%s'", $value );
			$exists = $this->db->get_var( $sql );
			echo $exists."\r\n";
			if ( !empty($exists)) {
				continue;
			}
			$this->db->insert( $this->table_name, array( $this->primary_key => $value ) );
			$i_count++;
		}
